...
const FloatingSkills = () => {
    ...
    () => {
      ...
      const ctx = gsap.context(() => {
        ...
        gsap.utils.toArray<HTMLElement>('.skill-float').forEach((el) => {
          gsap.to(el, {
            y:
              Math.random() > 0.5 ? `+=${Math.random() * 20 + 10}` : `-=${Math.random() * 20 + 10}`,
  ...
  return (
    ...
    >
      {/* Background glow */}
      <div
        className="absolute top-1/2 left-1/2 w-[300px] sm:w-[400px] md:w-[600px] h-[300px] sm:h-[400px] md:h-[600px] bg-accent/5 rounded-full blur-[80px] sm:blur-[100px] -translate-x-1/2 -translate-y-1/2 pointer-events-none"
        aria-hidden="true"
      />

      <h2 className="text-[10px] sm:text-xs text-accent font-bold tracking-[0.2em] sm:tracking-[0.3em] uppercase mb-3 sm:mb-4 relative z-10">
        Tech Stack
      </h2>
      <p className="text-xs sm:text-sm text-white/40 mb-8 sm:mb-12 md:mb-16 relative z-10">
        Hover to explore
      </p>
          ...
          return (
            <div key={skill.name} className="skill-wrapper inline-block relative group">
              <div className="skill-float inline-block relative">
                ...
                >
                  <span
                    className={`font-display font-black tracking-tighter ${sizeClass} ${isFilled ? 'text-white' : 'text-stroke opacity-70'} pointer-events-none`}
                  >
                    {skill.name}
                  </span>
                </div>
                ...
                >
                  ...
                  <div
                    className={`text-[10px] sm:text-xs font-bold uppercase tracking-wider ${getCategoryColor(skill.category)}`}
                  >
                    {skill.category}
                  </div>
      ...
      <div className="mt-10 sm:mt-16 flex flex-wrap justify-center gap-4 sm:gap-6 text-[10px] sm:text-xs uppercase tracking-wider relative z-10">
        <div className="flex items-center gap-1.5 sm:gap-2">
          <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-accent rounded-full" /> Language
        </div>
        <div className="flex items-center gap-1.5 sm:gap-2">
          <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-accent-2 rounded-full" /> Frontend
        </div>
        <div className="flex items-center gap-1.5 sm:gap-2">
          <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-green-400 rounded-full" /> Backend
        </div>
        <div className="flex items-center gap-1.5 sm:gap-2">
          <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-orange-400 rounded-full" /> Tool
        </div>
      </div>
    </section>
  );
